...
import 'package:mobileapp/widgets/loadmore/load_more_delegate_custom.dart';
import 'package:mobileapp/widgets/no_result_widget.dart';
...
class _LoadMoreState extends State<LoadMore> {
  ...
  LoadMoreDelegate get loadMoreDelegate => LoadMoreDelegateCustom();
  ...
  bool get isLeafSkeleton => widget.isLeafSkeleton ?? false;

  @override
  void initState() {
    super.initState();
    if (widget.child is GridView) {
      var gridView = widget.child as GridView;
      gridViewController = gridView.controller ?? ScrollController();

      gridViewController?.addListener(() {
        if (gridViewController!.position.extentAfter < 500) {
          setState(() {
            isEnd = true;
          });
        }
  ...
  ValueNotifier<LoadMoreStatus> _loadMoreStatus = ValueNotifier(LoadMoreStatus.idle);
  ...
  Widget? _buildGridView(GridView gridView) {
    ...
    if (delegate is SliverChildBuilderDelegate) {
      SliverChildBuilderDelegate delegate = gridView.childrenDelegate as SliverChildBuilderDelegate;
      if (!widget.whenEmptyLoad && delegate.estimatedChildCount == 0) {
        break outer;
      }

      var viewCount = (delegate.estimatedChildCount ?? 0);
      IndexedWidgetBuilder builder = (context, index) {
        final item = delegate.builder(context, index) ?? Container();
        if (isLeafSkeleton) {
          return Skeleton.leaf(enabled: isLoading, child: item);
        }
        return item;
      };

      if (viewCount == 0) return _renderNoResultWidget();
      ...
      return CustomScrollView(
        ...
        slivers: <Widget>[
          SliverPadding(
            padding: gridView.padding ?? padding(),
          ...
          SliverToBoxAdapter(
            child: isEnd ? _buildLoadMoreView() : SizedBox(),
          ),
        ],
      );
    }
  ...
  Widget? _buildListView(ListView listView) {
    ...
    if (delegate is SliverChildBuilderDelegate) {
      ...
      if (viewCount == 1) return _renderNoResultWidget();
